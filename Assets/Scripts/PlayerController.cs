...
public class PlayerController : MonoBehaviour {
    ...
    void FixedUpdate()
    {
         ...
         Vector3 m_Movement = new Vector3(Input.acceleration.x, 0, Input.acceleration.y);
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.CompareTag("Pick up"))
        {
            ...
            if(Application.loadedLevelName == "FollowThePath" && count==1)
            {
                
                Application.LoadLevel("GreatJob");

               
                
            }

            if(Application.loadedLevelName == "Sample2" && count==5)
            {
